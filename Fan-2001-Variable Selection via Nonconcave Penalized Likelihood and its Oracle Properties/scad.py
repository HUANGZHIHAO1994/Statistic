...
class SCADFit(object):
    """
    COORDINATE DESCENT ALGORITHMS FOR NONCONVEX PENALIZED REGRESSION,
    WITH APPLICATIONS TO BIOLOGICAL FEATURE SELECTION

    Reference: Breheny P and Huang J (2011). Coordinate descent algorithms for nonconvex penalized regression, with applications to biological feature selection. Annals of Applied Statistics, 5: 232–253
    """
    ...
    def fit(self):
        ...
        for m in range(self.iteration):
            self.Z = np.dot(self.reg_x.T, self.resid) / self.n + self.params
            self.params = np.array(list(map(self.__fscad, self.Z)))
            self.resid = self.reg_y - np.dot(self.reg_x, self.params)
        self.intercept = self.ymean - np.sum(self.params * self.xmean / np.sqrt(self.xvar))
        self.coef = self.params / np.sqrt(self.xvar)

    def predict(self, x_pre):
        return np.dot(x_pre, self.coef) + self.intercept
